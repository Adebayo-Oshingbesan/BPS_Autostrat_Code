...
class MDSS(object):
    ...
    def scan(self, coordinates: pd.DataFrame, expectations: pd.Series, outcomes: pd.Series, penalty: float,
                    num_iters: int, verbose: bool = False, seed: int = 0, mode: str = 'binary', starting_subset = None):
        """
        :param coordinates: data frame containing having as columns the covariates/features
        :param expectations: data series containing the expectations/expected outcomes
        :param outcomes: data series containing the outcomes/observed outcomes
        ...
        for i in range(num_iters):
            ...
            current_subset = starting_subset if (i == 0) \
    ...
    def parallel_scan(self, coordinates: pd.DataFrame, expectations: pd.Series, outcomes: pd.Series, penalty: float,
                    num_iters: int, verbose: bool = False, seed: int = 0, mode: str = 'binary', cpu = 0.75):
        """
        Run scan on multiple cores if they are available

        """
        num_processes = int(cpu_count() * cpu)
        np.random.seed(seed)

        if num_processes > 1:
            starting_subsets = [
                get_random_subset(
                    coordinates, np.random.rand(1).item(), 10
                )
                for _ in range(num_iters - 1)
            ]
            starting_subsets.append({})
            scan = partial(self.scan, coordinates,expectations, \
                    outcomes, penalty, 1, verbose, \
                    seed, mode)

            with ProcessPoolExecutor(max_workers=num_processes) as executor:
                results = executor.map(scan, starting_subsets)
